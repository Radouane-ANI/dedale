...
public class RandomWalkBehaviour extends TickerBehaviour{
	...
	public void onTick() {
		...
		// System.out.println(this.myAgent.getLocalName()+" -- myCurrentPosition is:
		// "+myPosition);
		if (myPosition != null) {
			...
			// Random r= new Random();
			// int moveId=1+r.nextInt(lobs.size()-1);//removing the current position from
			// the list of target, not necessary as to stay is an action but allow quicker
			// random move

			// Move to the first node without an agent
			int moveId = 0;// stay
			for (int i = 1; i < lobs.size(); i++) {
				boolean hasAgent = false;
				for (Couple<Observation, String> obs : lobs.get(i).getRight()) {
